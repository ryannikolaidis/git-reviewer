...
test-cov: install-dev ## Run tests with coverage (auto-installs dev dependencies)
	uv run pytest --cov=git_reviewer --cov-report=html --cov-report=term

lint: install-dev ## Run linters (auto-installs dev dependencies)
	uv run ruff check git_reviewer tests
	uv run pyright git_reviewer

lint-all: install-dev ## Run linters including tests (auto-installs dev dependencies)
	uv run ruff check git_reviewer tests
	uv run pyright git_reviewer tests

tidy: install-dev ## Fix formatting and linting issues (auto-installs dev dependencies)
	uv run ruff format git_reviewer tests
	uv run ruff check --fix git_reviewer tests
	uv run black git_reviewer tests

check: lint ## Run all checks (alias for lint)

version-dev: ## Bump development version
	@current_version=$$(grep -E "^__version__ = " git_reviewer/__init__.py | cut -d '"' -f2); \
	if [[ $$current_version =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)$$ ]]; then \
		...
		sed -i '' 's/__version__ = ".*"/__version__ = "'$$new_version'"/' git_reviewer/__init__.py; \
...
docker-build: ## Build Docker image
	docker build -t git-reviewer:latest .

docker-run: ## Run Docker container
	docker run --rm -it git-reviewer:latest
